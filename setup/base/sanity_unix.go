...
func PlatformSanityChecks() {
	// Zendrite needs a relatively high number of file descriptors in order
	...
	warn := func(rLimit *syscall.Rlimit) {
		logrus.Warnf("IMPORTANT: Process file descriptor limit is currently %d, it is recommended to raise the limit for Zendrite to at least 65535 to avoid issues", rLimit.Cur)
	}
